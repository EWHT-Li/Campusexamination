...
{
    rootUdpPort=8001;//3001;
    multicastAddress="236.0.0.26";
    goalPort=988;//3000;988
...
//    按摩端需求
    ...
    btMassage.setSingleShot(true);
    connect(&btMassage,SIGNAL(timeout()),this,SLOT(onBTMassageStar()));

    //face需求
    quint16 tempQuint16=8002;
...
void LiLANMangeServer::onRootUdpNews()
{
    while (rootUdpSocket->hasPendingDatagrams()) {
        ...
        if(!udpaClientRegistery->contains(&senderAddress))
        {
...
//    qDebug()<<"asd1"+tcpNews;
    ...
    else if(tcpSocketRegistery->value(tcpSocketDescript)->returnGoalName()=="MassageFace")
    {
...
//脸部需求
void LiLANMangeServer::sendTcpNewstoFace(QString goalName, QString str)
{
    sendTcpNews(goalName,str);
    massageOrder=str;
    btMassageName=goalName;
    btMassage.start(100);
}

void LiLANMangeServer::onBTMassageStar()
{
    sendTcpNews(btMassageName,massageOrder);
    qDebug()<<"第二春 ："+massageOrder;
}
